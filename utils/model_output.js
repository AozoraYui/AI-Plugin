...
export function hasUnsupportedToolResultClaim(text, options = {}) {
    ...
    if (!value) return false
    const completionClaim = /(?:我(?:已经|已|刚刚|刚才)?(?:为你|帮你)?(?:完成|执行|运行|调用|查看|读取|检查|搜索|查询|安装|补全|补齐|修复|编译|构建|部署|发送|上传|修改|调整|设置|切换|开启|关闭|重启|停止)|(?<!是否)(?<!能否)(?<!可否)已经(?:为你|帮你)?(?:完成|执行|运行|调用|查看|读取|检查|搜索|查询|安装|补全|补齐|修复|编译|构建|部署|发送|上传|修改|调整|设置|切换|开启|关闭|重启|停止)|(?:操作|命令|指令|任务|依赖|模块|编译|构建|部署|文件|消息|配置|难度|状态)(?:已经|已)(?:成功)?(?:完成|执行|运行|安装|补全|补齐|修复|编译|构建|部署|发送|上传|修改|调整|设置|切换|开启|关闭|重启|停止)|(?<!是否)(?<!能否)(?<!可否)(?:已|已经)(?:成功)?(?:切换到|运行了|执行了|读取了|查看了|搜索了|安装了|补全了|修复了|编译了|发送了|上传了|修改了|调整了|设置了|开启了|关闭了|重启了|停止了)|(?:我|机器人|诺亚)(?:正在|现在开始|马上开始)(?:为你|帮你)?(?:执行|运行|调用|修改|调整|设置|切换|开启|关闭|重启|停止))/i.test(value)
    const successConclusion = /(?:均已成功|已经修复好|已修复好|已经处理好|已处理好|执行成功|安装成功|编译成功|构建成功|部署成功|发送成功|上传成功|(?:已经|已)(?:成功)?(?:将|把)?.{0,20}(?:修改|调整|设置|切换|开启|关闭|重启|停止)(?:为|成|到|完成|成功|好了|完毕)|(?:当前|现在).{0,20}成功(?:修改|调整|设置|切换|开启|关闭|重启|停止)(?:为|成|到|完成|成功|好了|完毕)|任务完成)/i.test(value)
