...
export function hasUnsupportedToolResultClaim(text, options = {}) {
    ...
    if (!value) return false
    const completionClaim = /(?:我(?:已经|已|刚刚|刚才)?(?:为你|帮你)?(?:完成|执行|运行|调用|查看|读取|检查|搜索|查询|安装|补全|补齐|修复|编译|构建|部署|发送|上传|修改|调整|设置|切换|开启|关闭|重启|停止)|已经(?:为你|帮你)?(?:完成|执行|运行|调用|查看|读取|检查|搜索|查询|安装|补全|补齐|修复|编译|构建|部署|发送|上传|修改|调整|设置|切换|开启|关闭|重启|停止)|(?:操作|命令|指令|任务|依赖|模块|编译|构建|部署|文件|消息|配置|难度|状态)(?:已经|已)(?:成功)?(?:完成|执行|运行|安装|补全|补齐|修复|编译|构建|部署|发送|上传|修改|调整|设置|切换|开启|关闭|重启|停止)|(?:已|已经)(?:成功)?(?:切换到|运行了|执行了|读取了|查看了|搜索了|安装了|补全了|修复了|编译了|发送了|上传了|修改了|调整了|设置了|开启了|关闭了|重启了|停止了)|(?:我|机器人|诺亚)(?:正在|现在开始|马上开始)(?:为你|帮你)?(?:执行|运行|调用|修改|调整|设置|切换|开启|关闭|重启|停止))/i.test(value)
