...
| 指令 | 模型组 | 说明 |
|------|--------|------|
| `#gm [内容]` | Flash | 默认对话（速度快） |
| `#pgm [内容]` / `#progm [内容]` | Pro | Pro 模型对话（更聪明） |
| `#ugm [内容]` / `#ultragm [内容]` | Ultra | Ultra 模型对话（旗舰） |

#### 单次对话（不保存历史）
| 指令 | 模型组 | 说明 |
|------|--------|------|
| `#sgm [内容]` | Flash | 单次对话（不保存历史） |
| `#psgm [内容]` / `#prosgm [内容]` | Pro | Pro 单次对话 |
| `#usgm [内容]` / `#ultrasgm [内容]` | Ultra | Ultra 单次对话 |
...
> - 短命令 `pgm`/`ugm` 和完整命令 `progm`/`ultragm` 等效
...
|------|------|
| `#gemini记忆列表` | 查看记忆总结列表 |
| `#gemini读取记忆 [日期]` | 读取指定日期的记忆记录 |
| `#gemini创建全量总结` | 创建完整记忆总结（支持分块合并） |
| `#gemini创建增量总结` | 创建增量记忆总结 |
| `#gemini批量增量总结` | 批量处理未总结的日期 |

> 💡 记忆命令同样支持模型组前缀：`#pgemini`/`#progemini` → Pro，`#ugemini`/`#ultragemini` → Ultra
...
| `#gemini权限列表` | 查看当前权限配置 |
| `#gemini信任群添加 [群号]` | 添加信任群 |
| `#gemini信任群删除 [群号]` | 删除信任群 |
| `#gemini信任群列表` | 查看信任群列表 |
| `#gemini思考开启/关闭` | 开启/关闭思考过程显示 |
| `#gemini插件更新` | 检查并更新插件（git pull） |
| `#gemini插件强制更新` | 强制更新插件（git reset + pull） |
| `#gemini帮助` / `#ai帮助` | 显示帮助信息 |
...
10. **插件更新**：使用 `#gemini插件更新` 一键 git pull，`#gemini插件强制更新` 强制覆盖本地修改
