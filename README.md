...
| `#gemini添加白名单用户[ID]` | 添加白名单用户 |
| `#gemini删除白名单用户[ID]` | 删除白名单用户 |
| `#gemini添加白名单群[群号]` | 添加白名单群 |
| `#gemini删除白名单群[群号]` | 删除白名单群 |
...
| `gemini_user_profiles.json` | 用户档案 |
| `ai_plugin.db` | SQLite 数据库（对话历史、用户档案） |

### 数据存储
- **SQLite 数据库**：主要存储引擎，存储对话历史和用户档案
- **JSON 文件**：兼容旧数据，首次启动时自动迁移到 SQLite（旧文件保留不删除）
- **Redis**：可选缓存层，加速对话读取
