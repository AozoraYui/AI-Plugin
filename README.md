...
- **个人档案**：每次全量/增量总结成功后，会比对旧档案与新总结，维护一份更稳定的用户画像供后续对话理解
- 记忆档案列表查看与读取
- 支持导出个人/全部记忆
...
# NOA_CHAT_CONTEXT_LIMIT: 60          # 每次回复读取最近 60 条本群畅聊流水；unlimited / -1 为不限制
...
>  普通 `#chat` 会自动带上当前群最近监听流水，便于无缝接上群聊上下文；`NOA_CHAT_CONTEXT_LIMIT` 可设为 `unlimited` 或 `-1` 尽量读取全部流水；跨群/所有群流水仍仅主人可查。
...
>  全量/增量总结成功后会同步更新个人档案；个人档案只用于后续对话理解用户，不会作为公开群聊内容主动透露。
