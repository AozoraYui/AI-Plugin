...
# NOA_CHAT_CONTEXT_LIMIT: 60          # 畅聊/群聊记录查询读取最近 60 条；unlimited / -1 为不限制。普通 #c 自动上下文会内部限量，避免一次塞入过多流水
...
>  普通 `#chat` 会自动带上最近监听流水，便于无缝接上群聊上下文；这段自动上下文会内部限量，避免几千条记录淹没当前问题。明确询问某群/跨群记录时会调用 `group_chat_context` 单独查询，跨群/所有群流水仍仅主人可查。
