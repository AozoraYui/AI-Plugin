...
# NOA_CHAT_CONTEXT_LIMIT: 60          # 畅聊/群聊记录查询读取最近 60 条；unlimited / -1 为不限制。普通 #c 自动上下文和畅聊最终回复会内部限量/分段摘要，避免一次塞入过多流水
...
>  `NOA_CHAT_CONTEXT_LIMIT: unlimited` 只表示数据库/工具层尽量允许读取更多记录；最终发给模型的 prompt 会在过大时先分段摘要再提交汇总，并保留硬上限兜底，避免百万 token 请求。
