...
> Agent 任务只记录 `#chat/#c` 中进入工具链的任务，不会把普通闲聊都建成任务。主模型会标记单步/多步任务和可验证的成功标准；每轮工具结果由观察器判断为继续、完成、等待或阻塞。可恢复的工具失败会允许调整参数或更换只读工具继续规划，高风险工具仍按原权限和二次确认机制执行。
> 开发者可运行 `npm run eval:agent` 执行内置 Agent 路由、安全和循环决策回归评测。
