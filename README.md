...
- 畅聊读图有阈值：纯图片和图文混合消息都会自动触发读图；当前消息不受历史图片读取上限截断，单条消息不超过 `FAST_CHAT_DIRECT_IMAGE_LIMIT` 时直接交给多模态模型并保存视觉摘要，超过后按批次生成并保存图片摘要；回复冷却期仍会后台识图入库，只是不额外刷屏；历史上下文优先复用已保存摘要，未有摘要时才按配置临时预读
...
# FAST_CHAT_TRIGGER_ON_IMAGE: true     # 纯图片和图文混合消息自动触发读图；冷却期仍后台识图入库
...
>  当前纯图片和图文混合消息都会完整进入读图链路，不受 `FAST_CHAT_AUTO_READ_IMAGE_LIMIT` 或历史图片数量上限截断；不超过 `FAST_CHAT_DIRECT_IMAGE_LIMIT` 时直接读，更多图片进入分批摘要。
