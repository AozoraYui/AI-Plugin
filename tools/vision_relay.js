...
async function relayImagesToVision(imageUrls, context, client, visionModelConfig) {
    ...
    try {
        ...
        const promptText = context
            ? `请按顺序描述以下图片。用户消息中已用 [图片#N] 标记了每张图片的编号，请在你的描述中也使用相同的编号格式，例如"图片#1: ..."、"图片#2: ..."。请极尽详细地描述每张图片中的所有内容，包括但不限于：人物外貌/穿着/表情/动作、场景环境、物体细节、文字内容、颜色、构图等。用户附带的消息是：「${context}」，请在描述时注意关联用户意图。`
            : '请按顺序描述以下图片，用"图片#1:"、"图片#2:"的格式分别描述每张图片。请极尽详细地描述每张图片中的所有内容，包括但不限于：人物外貌/穿着/表情/动作、场景环境、物体细节、文字内容、颜色、构图等。'
