...
export const groupChatContextTool = {
    ...
    permission: 'everyone',
    description: '读取畅聊模式捕获的群消息流水，或列出机器人可见/已捕获群列表。普通 #c 中用户自然询问“他们刚才聊了啥/群里刚刚发生了什么/最近前情”即可使用。默认只读取当前群；用户问“我在别的群刚说了什么/你看到我其他群的消息吗”时可用 scope=my_recent_messages 或 other_group_messages 只查该用户自己的跨群消息；只有主人可用 scope=group_list、all_groups 或指定其他 group_id 查询所有已捕获群。工具本体只返回文本和图片元信息；对话流程会在明确问图片或总结前情时按配置临时预读图片摘要。',

    functionSchema: {
        type: 'function',
        function: {
            name: 'group_chat_context',
            description: '读取畅聊模式捕获的群聊流水，或列出机器人可见/已捕获群。普通 #c 中用户自然询问“他们刚才聊了啥/群里刚刚发生了什么/最近前情”即可读取当前群。默认当前群；可安全查询当前用户自己的跨群消息；主人可跨群查询全部已捕获公开流水。结果若含图片元信息，最终对话流程可按配置临时预读图片摘要。',
            parameters: {
                type: 'object',
                properties: {
                    scope: {
                    ...
                    hours: {
                        type: 'number',
                        description: '可选，只读取最近多少小时的消息，范围 1-168。用户明确说“最近几个小时/过去 N 小时”时填写。'
                    }
                },
                required: []
            }
    ...
    async execute(args = {}, context = {}) {
        ...
        if (scope === 'current_group') {
            ...
            logs = db.getGroupMessageLogs
                ? await db.getGroupMessageLogs({ groupId: effectiveGroupId, limit, query: effectiveQuery, sinceHours: hours })
    ...
    formatResult(data) {
        ...
        const timeNote = data.hours ? `，最近 ${data.hours} 小时` : ''
        ...
        if (data.scope === 'group_list') {
            ...
            if (groups.length === 0) {
                ...
                return `\n\n【群聊上下文】没有查到机器人可见或已捕获的群${queryNote}${timeNote}。${errNote}`
            }
            ...
            return `\n\n【群聊上下文】机器人可见/已捕获群共 ${groups.length} 个${queryNote}${timeNote}：${errNote}\n${lines.join('\n')}`
        }

        if (!Array.isArray(data.logs) || data.logs.length === 0) {
            return `\n\n【群聊上下文】${scopeName}最近没有可用记录${queryNote}${timeNote}。${privacyNote}${resolvedNote}`
        }
        ...
        return `\n\n【群聊上下文】${scopeName}最近命中 ${data.count} 条${groupNote}${queryNote}${timeNote}：${privacyNote}${resolvedNote}\n${lines.join('\n')}`
    }
}
