...
- file_download: {"save_dir": "可选，保存目录", "no_timestamp": "可选true/false", "rename_sequential": "可选true/false"}
  - 文件下载要求：当用户要求把当前消息或其引用消息里的图片/视频/语音/文件"下载/保存到服务器"时使用。无需填 URL，工具会自动从消息中提取媒体。save_dir 可省略。
  - 当用户说"不要时间戳目录/直接存到这个目录/就放在xxx目录下"时，把 no_timestamp 设为 true（直接存进 save_dir，不再建时间戳子目录）。
