...
- group_file_list: {"folder_name": "可选，子文件夹名"}
  - 群文件浏览要求：当用户想"看看群文件有哪些/列一下群文件/群文件里有什么"时使用。要进某个子文件夹就填 folder_name，否则留空看根目录。注意这是 QQ"群文件区"，不是聊天消息里的文件。
- group_file_download: {"file_name": "群文件名", "save_dir": "可选，保存目录"}
  - 群文件下载要求：当用户要求把"群文件里的某个文件"下载/保存到服务器某目录时使用。file_name 填用户说的文件名（可为片段）。这是从 QQ 群文件区下载，区别于 file_download（后者下载聊天消息里的媒体）。
- draw_image: {"prompt": "画图描述", "preset": "可选预设名", "quality": "可选 flash/pro/ultra"}
