...
- group_file_download: {"file_name": "可选，群文件名", "save_dir": "可选，保存目录"}
  - 群文件下载要求：当用户要求把"群文件里的某个文件"下载/保存到服务器某目录时使用。file_name 填用户说的文件名（可为片段）。若用户是"引用了一条群文件消息"再说"下载这个/帮我下载到xxx/把这个存到服务器"，file_name 可以留空，工具会自动从被引用的群文件消息里提取文件名。这是从 QQ 群文件区下载，区别于 file_download（后者下载聊天消息里的媒体）。
