...
🎨 创意作图:
  ...
  #画图预设列表pro - 查看详细预设
  #画图预设重载 - 重载预设配置(管理员)
  #画图预设添加 [指令] [名称] - 添加预设(管理员)
  #画图预设删除 [指令] - 删除预设(管理员)
  #添加预设别名 [指令] - 添加别名(管理员)
  #删除预设别名 [指令] - 删除别名(管理员)

📚 记忆归档:
  #gemini总结记忆列表 - 查看记忆档案目录
  #gemini创建增量锚点 - 快速存档(低消耗)
  #gemini创建全量锚点 - 完整重玩(高消耗)
...
🔑 主人专用:
  ...
  #gemini权限添加/删除 黑名单群[群号]
  #gemini状态 - 查看插件状态
  #gemini思考开启/关闭 - 切换思考过程显示
  #导出诺亚全部记忆 - 导出所有用户记忆(管理员)

💡 提示:
  - 所有对话指令都支持发送图片
  - 在指令前加"3"可调用Gemini3模型(如#3bnn)
  - 使用@某人可获取其头像进行作图
  - 支持合并转发消息展开
            `.trim()
