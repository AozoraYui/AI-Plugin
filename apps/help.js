...
> #${chatCmd}v [内容]
  ...
  全局开启后，仅当前模型组没有可用多模态模型时自动启用；有图请求会优先使用多模态模型。
> #${chatCmd}n [内容]
...
> 代发群消息（主人专用）
  ...
  显式列出多个目标时可批量；默认会加“【主人转达】”前缀；不支持 CQ 码；发送前会先创建待确认操作。
> 遥控退群（主人专用）
  需在 models_config.yaml 开启 enable_group_leave: true（独立开关），也可用 #ai开启退群 / #ai关闭退群 运行时切换。
...
> 畅聊模式（主人开关）
  ...
  捕获会覆盖所有非黑名单群，包括 # 开头命令消息（会标记为历史命令记录）。合并/嵌套合并消息会展开，过长会分段入库。
  畅聊记录会同步到普通对话记忆；畅聊与 #c 共享个人记忆、个人档案和向量召回核心，并可在安全范围内调用已开启的工具（高危工具仍按权限鉴权）。
  普通 #c 会自动带上最近监听流水，但自动注入会内部限量，避免大量记录淹没当前问题；明确询问某群/跨群记录时会单独查询。跨群/所有群流水仍仅主人可查。
  短前情会快速读取最近流水；问“最近几天/昨天/我不在的时候/从我上次说话后聊了什么”会走分段摘要工具，适合补课且避免 Token 爆炸。
  ...
  图片不保存本体；纯图片和图文混合消息会静默读图并保存视觉摘要，不会仅因发图自动回复。最多 ${fastChatDirectImageLimit} 张直接识别，更多图片按每批 ${fastChatImageBatchSize} 张摘要。
...
> #ai增量总结
  ...
  每16轮对话自动触发 + 每天23:50定时覆盖。
  总结成功后会比对旧个人档案与新总结，维护更稳定的用户画像供后续对话理解。
> #ai增量总结 [日期]
  为指定日期创建增量总结。
...
> #ai模型测试 模型ID/别名
  不带参数会逐个直连测试全部配置模型；指定模型时只测试该模型。绘图模型会真实调用图片生成接口。
  标注“按次”的为按次扣费模型，调度时会尽量避开，按量模型全部不可用时才降级使用。
> #ai禁用 [模型ID]
  禁用指定模型。
> #ai启用 [模型ID]
  启用已禁用的模型。
...
> 图片支持
  ...
  畅聊模式会静默读取纯图片和图文混合消息：不超过 ${fastChatDirectImageLimit} 张直接读，更多图片按每批 ${fastChatImageBatchSize} 张分批摘要；发图本身不会触发回复。
> 临时开关
  ...
  #cn 查天气 → 启用联网搜索
  #cvn 这是个啥 → 同时启用两者
  Shell 执行需 enable_shell_exec 开启（独立开关），且仅主人可用
